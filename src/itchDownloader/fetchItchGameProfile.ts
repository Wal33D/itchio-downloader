...
import {
  IParsedItchGameUrl,
  IParsedItchGameMetadata,
  IItchRecord,
  IItchGameProfileResponse,
} from './types';
...
export const fetchItchGameProfile = async ({
  ...
  if (!itchGameUrl) {
    throw new Error(
      'Insufficient parameters: either a full URL or both author and name must be provided.',
    );
  }
  ...
  try {
    metaData = await parseItchGameMetadata({
